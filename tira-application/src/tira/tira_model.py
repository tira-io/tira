...
def get_vm(vm_id: str):
    ...
    return model.vms.get(vm_id, None)


def get_tasks() -> list:
    tasks = [get_task(task.taskId)
             for task in model.tasks.values()]
    return tasks


def get_run(dataset_id: str, vm_id: str, run_id: str, return_deleted: bool = False) -> dict:
    run = model.load_run(dataset_id, vm_id, run_id, return_deleted)
...
def get_task(task_id: str) -> dict:
    ...
    return {"task_name": t.taskName,
            ...
            "software_count": len(model.software_by_task.get(t.taskId, {0})),
            "web": t.web,
            "organizer": model.organizers.get(t.hostId, modelpb.Hosts.Host()).name,
            "year": model.organizers.get(t.hostId, modelpb.Hosts.Host()).years
            }


def get_dataset(dataset_id: str) -> dict:
    dataset = model.datasets[dataset_id]
    return {
        ...
        "task": model.default_tasks.get(dataset.datasetId, "None"),
        'organizer': model.task_organizers.get(dataset.datasetId, ""),
        "software_count": model.software_count_by_dataset.get(dataset.datasetId, 0)
    }


def get_datasets() -> dict:
    """ Get a dict of dataset_id: dataset_json_descriptor """
    return {dataset_id: get_dataset(dataset_id) for dataset_id in model.datasets}
...
def get_datasets_by_task(task_id: str, include_deprecated=False) -> list:
    ...
    """
    return [get_dataset(dataset.datasetId)
            for dataset in model.datasets.values()
            if task_id == model.default_tasks.get(dataset.datasetId, "") and
            not (dataset.isDeprecated and not include_deprecated)]


def get_organizer(organizer_id: str):
    # TODO should return as dict
    return model.organizers[organizer_id]


def get_host_list() -> list:
    return list(open(model.host_list_file, "r").readlines())


def get_ova_list() -> list:
    return [f"{ova_file.stem}.ova" for ova_file in model.ova_dir.glob("*.ova")]
...
def get_vm_list():
    ...
    return list(parse_vm_list(open(model.vm_list_file, 'r')))
...
def get_vms_by_dataset(dataset_id: str) -> list:
    ...
    return [user_run_dir.stem
            for user_run_dir in (model.RUNS_DIR_PATH / dataset_id).glob("*")]


def get_vm_runs_by_dataset(dataset_id: str, vm_id: str, return_deleted: bool = False) -> list:
    runs = {}
    for run_id_dir in (model.RUNS_DIR_PATH / dataset_id / vm_id).glob("*"):
        run_id = run_id_dir.stem
        run = get_run(dataset_id, vm_id, run_id, return_deleted=return_deleted)
...
def get_vm_runs_by_task(task_id: str, vm_id: str, return_deleted: bool = False) -> list:
    ...
    relevant_datasets = {software["dataset"] for software in get_software(task_id, vm_id)}
    runs = []
    for dataset_id in relevant_datasets:
        runs.extend(get_vm_runs_by_dataset(dataset_id, vm_id, return_deleted=return_deleted))
    return runs
...
def get_vms_with_reviews(vm_ids: list, dataset_id: str, vm_reviews) -> list:
    ...
    """
    vm_runs = {vm_id: get_vm_runs_by_dataset(dataset_id, vm_id)
               for vm_id in vm_ids}
...
def get_evaluator(dataset_id, task_id=None):
    ...
    if task_id is None:
        task_id = model.default_tasks.get(dataset.datasetId, None)

    task = model.tasks.get(task_id)
    vm_id = task.virtualMachineId
    master_vm = Parse(open(model.vm_dir_path / f"{vm_id}.prototext", "r").read(),
...
def get_vm_evaluations_by_dataset(dataset_id, vm_id, only_public_results=True):
    ...
    """
    return {run_id: model.get_evaluation_measures(ev)
            for run_id, ev in
            model.load_vm_evaluations(dataset_id, vm_id, only_published=only_public_results).items()}
...
def get_evaluations_with_keys_by_dataset(vm_ids, dataset_id, vm_reviews=None):
    ...
    """
    vm_evaluations = {vm_id: get_vm_evaluations_by_dataset(dataset_id, vm_id,
...
def get_run_review(dataset_id: str, vm_id: str, run_id: str) -> dict:
    review = model.load_review(dataset_id, vm_id, run_id)
...
def get_vm_reviews_by_dataset(dataset_id: str, vm_id: str) -> dict:
    return {run_id_dir.stem: get_run_review(dataset_id, vm_id, run_id_dir.stem)
            for run_id_dir in (model.RUNS_DIR_PATH / dataset_id / vm_id).glob("*")}
...
def get_software(task_id, vm_id, software_id=None):
    ...
    sw = [{"id": software.id, "count": software.count,
          ...
          for software in model.software.get(f"{task_id}${vm_id}", [])]
...
def update_review(dataset_id, vm_id, run_id,
    ...
    """
    try:
        model.update_review(dataset_id, vm_id, run_id, reviewer_id, review_date, has_errors, has_no_errors, no_errors,
...
def update_run(dataset_id, vm_id, run_id, deleted: bool = None):
    ...
    """
    try:
        model.update_run(dataset_id, vm_id, run_id, deleted)
        return True
    except Exception as e:
...
def task_exists(task_id: str) -> bool:
    return task_id in model.tasks


def dataset_exists(dataset_id: str) -> bool:
    return dataset_id in model.datasets


def vm_exists(vm_id: str) -> bool:
    return vm_id in model.vms


def organizer_exists(organizer_id: str) -> bool:
    return organizer_id in model.organizers


def run_exists(vm_id: str, dataset_id: str, run_id: str) -> bool:
    return True if get_run(dataset_id, vm_id, run_id) else False
