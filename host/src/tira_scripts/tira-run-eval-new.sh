...
main() {
    ...
    if [ "$#" -ne 4 ]; then
        logError "[tira-run-eval-new] Wrong amount of parameters, see:"
        usage
    fi
    ...
    logInfo "[tira-run-eval-new] taskname: $taskname"

    # Define inputDataset.
    submissionFile=$(find "$submissionFileDir" -type f -name "$submissionFileName")

    logDebug "[tira-run-eval-new] Submission file name: $submissionFileName"
    logDebug "[tira-run-eval-new] Submission file dir: $submissionFileDir"
    logDebug "[tira-run-eval-new] Submission file: $submissionFile"

    # Check if submission file exists.
    if [ ! -e "$submissionFile" ]; then
        logError "[tira-run-eval-new] $submissionFile does not exist."
        usage
    fi
    ...
    logInfo "[tira-run-eval-new] $user@$host: copying input run into the virtual machine..."
    ...
    logInfo "[tira-run-eval-new] $user@$host: establishing SSH connection and executing command..."
    logInfo "[tira-run-eval-new] \t$cmd"
    ...
    logInfo "[tira-run-eval-new] $user@$host: copying $outputRunDir from VM..."
    ...
    logInfo "[tira-run-eval-new] $user@$host: removing $outputDir from VM..."
    sshpass -p "$userpw" ssh "$user@$host" -p "$sshport" -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no \
        -o LogLevel=error -t -t "rm -rf $outputRunDir; exit"

    # Remove inputRun on vm.
    logInfo "[tira-run-eval-new] $user@$host: removing /tmp/$inputRunName from VM..."
    sshpass -p "$userpw" ssh "$user@$host" -p "$sshport" -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no \
    ...
    logInfo "[tira-run-eval-new] $user@$host: done."
}
