    ...
    <v-container v-if="!loading">
      <v-card class="px2" max-width="2560">
        <v-card-item :title="task.task_name">
          <template v-slot:subtitle>by <a :href="link_organizer">{{ task.organizer }}</a> (<a
              :href="contact_organizer">contact</a>)</template>
        </v-card-item>
        ...
        <v-card-actions>
          <v-row>
            <v-col cols="6"><submit-button :task="task" :vm="vm" :user_id="user_id"
                :user_vms_for_task="user_vms_for_task" /></v-col>
