  ...
  <v-row v-if="!loading && userinfo.role !== 'guest'">
    ...
    <v-col v-if="!$vuetify.display.smAndDown" :cols="$vuetify.display.mdAndUp ? '3' : '0'">
      ...
      <v-btn color="primary" v-if="$vuetify.display.mdAndUp" prepend-icon="mdi-plus" size="large"
        @click="this.tab = 'newDockerImage'" block>New Submission</v-btn>
    </v-col>
  </v-row>
  <v-row v-if="$vuetify.display.smAndDown">
    <v-col :cols="12">
      <v-btn color="primary" prepend-icon="mdi-plus" size="large" @click="this.tab = 'newDockerImage'" block rounded>New
        Submission</v-btn>
    </v-col>
  </v-row>
  ...
  <v-window v-model="tab" v-if="!loading && userinfo.role !== 'guest'" :touch="{ left: null, right: null }">
    <v-window-item v-for="ds in this.docker.docker_softwares" :value="ds.docker_software_id">
      <existing-docker-submission @deleteDockerImage="handleDeleteDockerImage"
        @modifiedSubmissionDetails="v => handleModifiedSubmission(v, this.docker.docker_softwares)"
    ...
    <v-window-item value="newDockerImage">
      <new-docker-submission @add-new-docker-image="handleAddNewDockerImage" :is_ir_task="is_ir_task"
        :user_id_for_submission="user_id_for_submission" :step_prop="step"
        :docker_softwares="this.docker.docker_softwares" :organizer="organizer" :organizer_id="organizer_id" />
    </v-window-item>
  </v-window>
