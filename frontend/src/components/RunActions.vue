  ...
  <div :class="!$vuetify.display.mdAndUp ? ' d-flex flex-column w-100 justify-space-evenly' : 'd-flex'">
    ...
    <v-dialog v-if="userinfo.role === 'admin'">
      <template v-slot:activator="{ props }">
        <span>
          <v-btn :class="!$vuetify.display.mdAndUp ? 'pa0 ma0 d-none' : 'pa0 ma0'" icon="mdi-gavel" rounded
            density="compact" v-bind="props" />
          <v-btn class="pa0 ma0 mb-2 d-md-none d-flex justify-space-between" prepend-icon="mdi-gavel" rounded
            density="compact" min-width="180" v-bind="props">
            <p>Re-evaluate</p>
          </v-btn>
      ...
      <template v-slot:default="{ isActive }">
        <v-card class="pb-1">
          ...
          <v-card-actions class="justify-end">
            <v-row>
              <v-col cols="6"><v-btn variant="outlined" :loading="start_evaluation_is_pending"
                  @click="isActive.value = false" block>Close</v-btn></v-col>
              <v-col cols="6"><v-btn variant="outlined" :loading="start_evaluation_is_pending"
                  @click="runEvaluation(isActive)" block>Confirm</v-btn></v-col>
            </v-row>
          </v-card-actions>
        </v-card>
      </template>
    ...
    <span>
      <v-btn v-if="link_serp != null" icon="mdi-search-web" :href="link_serp" target="_blank"
        :class="!$vuetify.display.mdAndUp ? 'pa0 ma0 d-none' : 'pa0 ma0'" rounded density="compact" />
      <v-btn v-if="link_serp != null" prepend-icon="mdi-search-web" :href="link_serp" target="_blank"
        class="pa0 ma0 mb-2 d-md-none d-flex justify-space-between" min-width="180" rounded density="compact">
        <p>Show SERP</p>
      </v-btn>
    ...
    <span>
      <v-btn v-if="link_code != null" icon="mdi-code-json" :href="link_code" target="_blank"
        :class="!$vuetify.display.mdAndUp ? 'pa0 ma0 d-none' : 'pa0 ma0'" rounded density="compact" />
      <v-btn v-if="link_code != null" prepend-icon="mdi-code-json" :href="link_code" target="_blank"
        class="pa0 ma0 mb-2 d-md-none d-flex justify-space-between" min-width="180" rounded density="compact">
        <p>Show Code</p>
      </v-btn>
    ...
    <v-dialog v-if="run['owned_by_user']">
      <template v-slot:activator="{ props }">
        <span>
          <v-btn icon="mdi-delete" :disabled="!can_delete"
            :class="!$vuetify.display.mdAndUp ? 'pa0 ma0 d-none' : 'pa0 ma0'" rounded density="compact"
            v-bind="props" />
          <v-btn prepend-icon="mdi-delete" :disabled="!can_delete"
            class="pa0 ma0 d-md-none d-flex justify-space-between" min-width="180" rounded density="compact"
            v-bind="props">
            <p>Delete run</p>
          </v-btn>
          <v-tooltip activator="parent" location="top" v-if="!can_delete">You can not delete runs that are published
            and/or valid. Please contact the organizer to delete this run.</v-tooltip>
          <v-tooltip activator="parent" location="top" v-if="can_delete">Attention, this will delete this
            run.</v-tooltip>
        </span>
      </template>
      <template v-slot:default="{ isActive }">
        <v-card class="pb-1">
          ...
          <v-card-actions class="justify-end">
            <v-row>
              <v-col cols="6"><v-btn variant="outlined" :loading="delete_is_pending" @click="isActive.value = false"
                  block>Close</v-btn></v-col>
              <v-col cols="6"><v-btn variant="outlined" :loading="delete_is_pending" @click="deleteRun(isActive)"
                  block>Confirm</v-btn></v-col>
            </v-row>
          </v-card-actions>
        </v-card>
      </template>
...
export default {
  ...
  methods: {
    runEvaluation(isActive: any) {
      ...
      get(this.base_url + `/grpc/${this.run.vm_id}/run_eval/${this.run.dataset_id}/${this.run.run_id}`)
    ...
    deleteRun(isActive: any) {
      ...
      get(this.base_url + `/grpc/${this.run.vm_id}/run_delete/${this.run.dataset_id}/${this.run.run_id}`)
