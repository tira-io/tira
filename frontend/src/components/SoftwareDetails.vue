<template>
  <v-card v-if="!loading" flat class="px-0 mx-0">
    <v-select class="d-md-none" v-model="selectedComponentTab" label="Select item to get more information"
      :items="tabs"></v-select>
      ...
      <v-window v-model="component_tab" :touch="{ left: () => { }, right: () => { } }">
        <v-window-item value="details">
          <v-card-item class="d-md-none" v-if="run && run.link_to_team"><b>Team: </b>
            ...
            <a v-if="userinfo.role != 'admin'" target="_blank" :href="run.link_to_team">{{ run.vm_id }}</a>
            <v-menu v-if="userinfo.role == 'admin'" transition="slide-y-transition">
              <template v-slot:activator="{ props }">
                <a href="javascript:void(0)" v-bind="props">{{ run.vm_id }}</a>
              </template>
              <v-list>
                <v-list-item key="team-page"><a target="_blank" :href="run.link_to_team">Team Page of {{ run.vm_id
                    }}</a></v-list-item>
                <v-list-item key="submission-page"><a target="_blank"
                    :href="'/submit/' + task_id + '/user/' + run.vm_id">Submission
                    Page of {{ run.vm_id }}</a></v-list-item>
              </v-list>
            </v-menu>
          </v-card-item>
        ...
        <v-window-item value="reproduction" v-if="!cli_command || !python_command || !docker_command">
          <div class="ma-2">
            <v-card-item>TIRA allows to reproduce/replicate submitted software approaches via a single command using
              Docker.
              This software is not yet made publicly available.</v-card-item>
            <v-card-item>Please <a :href="contact_organizer">contact</a> the organizer <a :href="link_organizer"> {{
              organizer
                }}</a> or the team <a :href="run.link_to_team">{{ run.vm_id }}</a> if you want to have access to this
              software
              or its results, as they can decide (in consultation with each other) to make the software and/or its
              results
              publicly available via TIRA.</v-card-item>
          </div>
        </v-window-item>
