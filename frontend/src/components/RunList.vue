  ...
  <div v-if="!loading">
    <v-data-table v-if="showTable" v-model="selected_runs" show-expand :headers="table_headers" :items="runs"
      ...
      <template #item.vm_id="{ item }">
        ...
        <a v-if="userinfo.role != 'admin'" target="_blank" :href="item.link_to_team">{{ item.vm_id }}</a>

        <v-menu v-if="userinfo.role == 'admin'" transition="slide-y-transition">
          <template v-slot:activator="{ props }">
            <a href="javascript:void(0)" v-bind="props">{{ item.vm_id }}</a>
          </template>
          <v-list>
            <v-list-item key="team-page"><a target="_blank" :href="item.link_to_team">Team Page of {{ item.vm_id
                }}</a></v-list-item>
            <v-list-item key="submission-page"><a target="_blank"
                :href="'/submit/' + task_id + '/user/' + item.vm_id">Submission
                Page of {{ item.vm_id }}</a></v-list-item>
          </v-list>
        </v-menu>
      </template>
      ...
      <template v-slot:expanded-row="{ columns, item }">
        <tr>
          <td :colspan="columns.length" style="background-color: white;" class="px-0 mx-0">
            <software-details :run="item" :columns_to_skip="table_headers" :organizer="organizer"
    ...
    <v-data-table v-if="showTable" show-expand :headers="table_headers_small_layout" :items="runs" item-value="Run"
      v-model:sort-by="table_sort_by" expand-on-click density="compact" class="elevation-1 d-md-none" hover>
      <template #item.vm_id="{ item }">
      ...
      <template v-slot:expanded-row="{ columns, item }">
        <tr>
          <td :colspan="columns.length" style="background-color: white;" class="px-0 mx-0">
            <software-details :run="item" :columns_to_skip="table_headers_small_layout" :organizer="organizer"
              :organizer_id="organizer_id" @reviewRun="(i: any) => reviewChanged(i)" />
          </td>
        </tr>
      </template>
    </v-data-table>
    ...
    <div v-if="showTable" class="d-none d-md-block">
      <v-row class="pt-2">
        <v-col cols="6"><v-btn variant="outlined" block :disabled="downloadLink === ''" :href="downloadLink"
            target="_blank">Download Selected</v-btn></v-col>
        <v-col cols="6"><v-btn variant="outlined" block :disabled="compareLink === ''" :href="compareLink"
            target="_blank">Compare Selected</v-btn></v-col>
      </v-row>
    </div>
  </div>
</template>
