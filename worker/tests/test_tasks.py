...
class TestRunTask(unittest.TestCase):
    ...
    @patch.object(_tasks, "resolve_dynamic_mounts", return_value=None)
    @patch.object(_tasks, "get_admin_client")
    def test_run_copies_network_access_log_next_to_output_dir_into_output_dir(
        self,
        get_admin_client,
        resolve_dynamic_mounts,
        execute_monitored,
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            output_dir.mkdir()
            (Path(tmp_dir) / "network-access.log").write_text("example.com\t3\n")
            execute_monitored.return_value = output_dir
            ...
            copied_log = output_dir / "network-access.log"
            self.assertTrue(copied_log.is_file())
            self.assertEqual("example.com\t3\n", copied_log.read_text())
            client.upload_run_admin.assert_called_once_with(output_dir, "job-1")
    ...
    @patch.object(_tasks, "resolve_dynamic_mounts", return_value=None)
    @patch.object(_tasks, "get_admin_client")
    def test_run_does_not_fail_when_no_network_access_log_exists(
        self,
        get_admin_client,
        resolve_dynamic_mounts,
        execute_monitored,
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertFalse((output_dir / "network-access.log").exists())
            client.upload_run_admin.assert_called_once_with(output_dir, "job-1")
