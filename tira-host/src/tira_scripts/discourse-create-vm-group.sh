...
invite_users() {
    ...
    curl -X POST "${_CONFIG_tira_disraptor_url}/invites" -H "Api-Key: $api_key" -H "Accept: application/json" -H "Content-Type: multipart/form-data" -F "group_ids[]=$group_id" -F "max_redemptions_allowed=20" -F "expires_at=$year-12-31"|jq '.link'
}
