...
from tira.model import EvaluationLog, TransactionLog, TransitionLog
...
class TiraApplicationService(tira_host_pb2_grpc.TiraApplicationService):
    def set_state(self, request, context):
        ...
        try:
            ...
            _ = TransitionLog.objects.update_or_create(
                vm_id=request.vmId, defaults={"vm_state": request.state, "transaction": t}
            )
    ...
    def confirm_vm_create(self, request, context):
        ...
        _ = TransactionLog.objects.filter(transaction_id=request.transaction.transactionId).update(
            completed=False, last_status=str(request.transaction.status), last_message=request.transaction.message
        )
        ...
        if request.transaction.status == tira_host_pb2.Status.SUCCESS:
            ...
            logger.error(
                "Application received confirm_vm_create with status Failed:\n"
