...
def load_refresh_timestamp_for_cache_key(cache: BaseCache, key: str) -> datetime.datetime:
    try:
        ...
        with connection.cursor() as cursor:
            ...
            ret = cursor.fetchall()
            if len(ret) > 0:
                assert isinstance(ret[0][0], datetime.datetime)
...
def discourse_api_client() -> DiscourseApiClient:
    return DiscourseApiClient(url=settings.DISCOURSE_API_URL, api_key=settings.DISRAPTOR_API_KEY)


def tira_run_command(image: str, command: str, task_id: str) -> str:
...
def tira_docker_registry_token(docker_software_help: str) -> tuple[str, str]:
...
def github_user_exists(user_name: str) -> bool:
    g = get_git_runner_for_software_integration()
    return g.git_user_exists(user_name)


def get_discourse_token_for_user(vm_id: str, disraptor_user: str) -> "Optional[str]":
...
def get_submission_git_repo(
    ...
    assert docker_data is not False
    ...
    disraptor_description = f"{disraptor_user}-repo-{task_id}-{vm_id}"
    discourse_api_key = discourse_api_client().generate_api_key(disraptor_user, disraptor_description)
    ...
    submission = model.get_submission_git_repo_or_none(repository_url, vm_id, return_object=True)
    assert submission is not None

    assert isinstance(settings.CODE_SUBMISSION_REPOSITORY_NAMESPACE, str)
    assert external_owner is not None
    assert disraptor_user is not None
...
def get_runs_for_vm(vm_id: str, docker_software_id: str, upload_id: str) -> "tuple[list[str], list[dict[str, Any]]]":
    ...
    return model.get_runs_for_vm(vm_id, dsid, upid)


def get_docker_software_by_name(name: str, vm_id: str, task_id: str) -> "dict[str, Any]":
...
def get_organizer(organizer_id: str) -> "dict[str, Any]":
...
def get_evaluations_of_run(vm_id: str, run_id: str) -> list:
...
def get_evaluations_with_keys_by_dataset(
    dataset_id: str, include_unpublished: bool = False, show_only_unreviewed: bool = False
...
def get_job_details(task_id: str, vm_id: str, job_id: str) -> "Optional[dict[str, Any]]":
    return model.get_job_details(task_id, vm_id, job_id)


def get_evaluation(run_id: str) -> dict[str, str]:
...
def get_count_of_missing_reviews(task_id: str) -> list[dict[str, str]]:
...
def import_submission(task_id: str, vm_id: str, submission_type: str, s_id: str) -> None:
    return model.import_submission(task_id, vm_id, submission_type, s_id)


def get_upload(task_id: str, vm_id: str, upload_id: str) -> "dict[str, Any]":
...
def get_run_review(dataset_id: "Optional[str]", vm_id: "Optional[str]", run_id: str) -> dict:
...
def add_uploaded_run(
    task_id: str, vm_id: str, dataset_id: str, upload_id: str, uploaded_file: "UploadedFile"
) -> "dict[str, Any]":
...
def add_docker_software_mounts(docker_software: "dict[str, Any]", mounts: str) -> None:
...
def add_docker_software(
    ...
    image: str,
    command: str,
    software_inputs: "Optional[str]" = None,
...
def add_registration(data: dict[str, str]) -> None:
    model.add_registration(data)


def all_allowed_task_teams(task_id: str) -> set[str]:
    task = get_task(task_id)
    return set([i.strip() for i in task["allowed_task_teams"].split() if i.strip()])


def user_is_registered(task_id: str, request: "HttpRequest") -> bool:
...
def remaining_team_names(task_id: str) -> list[str]:
...
def add_vm(vm_id: str, user_name: str, initial_user_password: str, ip: str, host: str, ssh: str, rdp: str) -> None:
...
def add_software(task_id: str, vm_id: str) -> "dict[str, Any]":
...
def add_run(dataset_id: str, vm_id: str, run_id: str) -> str:
...
def delete_docker_software(task_id: str, vm_id: str, docker_software_id: str) -> bool:
    """
...
def delete_run(dataset_id: str, vm_id: str, run_id: str) -> bool:
...
def create_re_rank_output_on_dataset(
    ...
    if not is_ir_task or not irds_re_ranking_image or not irds_re_ranking_command or not irds_re_ranking_resource:
        return None
    docker = model.get_irds_docker_software_id(task_id, vm_id, software_id, docker_software_id)
    assert docker is not None
    docker_irds_software_id = docker.docker_software_id
...
def get_all_reranking_datasets(force_cache_refresh: bool = False) -> "dict[str, Any]":
    cache_key = "get_all_reranking_datasets"
    ret: dict[str, Any] = cache.get(cache_key)
    if ret is not None and not force_cache_refresh:
        return ret
...
def all_registrations(task_id: str) -> "list[dict[str, Any]]":
