...
from ...tira_model import get_all_reranking_datasets, get_git_integration
...
class Command(BaseCommand):
            ...
            for task in model.get_tasks():
                ...
                if model.git_pipeline_is_enabled_for_task(task["task_id"], cache):
                    ...
                    repositories = set(
                        [
                            i["git_repository_id"]
                            for i in evaluators_for_task
                            if i["is_git_runner"] and i["git_repository_id"]
                        ]
                    )
                    ...
                    for git_repository_id in repositories:
                        try:
                            ...
                            running_pipelines = git_integration.all_running_pipelines_for_repository(
                                git_repository_id, cache, force_cache_refresh=True
                            )
                            print(
                                "Refreshed Cache ("
                            ...
                            logger.warning(
                                f"Exception during refreshing the repository {git_repository_id}", exc_info=e
                            )
                            continue
    ...
    def refresh_user_images_in_repo(self, git_runner, sleep_time):
        ...
        for task in model.get_tasks():
            ...
            if "featured" in task and task["featured"] and "allowed_task_teams" in task and task["allowed_task_teams"]:
                users_of_active_tasks |= set(
                    [i.strip() for i in task["allowed_task_teams"].split("\n") if i and i.strip()]
                )

        print(
            str(datetime.datetime.now())
            + ": Start loop to keep the user images fresh (sleeped "
            + str(int(sleep_time))
            + f" seconds) for {users_of_active_tasks} ...",
            flush=True,
        )
    ...
    def keep_user_images_fresh(self, sleep_time):
        while True:
            ...
            print(
                str(datetime.datetime.now())
                + ": Start loop over all git runners to keep user images fresh (sleeped "
                + str(int(sleep_time))
                + " seconds) ...",
                flush=True,
            )
    ...
    def keep_reranking_datasets_fresh(self, sleep_time):
        while True:
            ...
            print(
                str(datetime.datetime.now())
                + ": Start keep_reranking_datasets_fresh (sleeped "
                + str(int(sleep_time))
                + " seconds) ..."
            )
            try:
