...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        help="Output file",
        default=str((pathlib.Path(__file__).parent.resolve() / "oracle-index.jsonl.gz").absolute()),
        required=False,
    )
    parser.add_argument(
        "--fb-terms", type=list, help="fb_terms passed to pyterrier", nargs="+", default=[10, 20, 30], required=False
    )
    parser.add_argument(
        "--w-models",
        type=list,
        help="weighting models passed to pyterrier",
        nargs="+",
        default=["BM25", "DirichletLM"],
        required=False,
    )
    parser.add_argument("--oracle-dataset-ids", type=str, help="allowed dataset ids", nargs="+", required=True)
    parser.add_argument(
        "--fb-docs", type=list, help="fb_docs passed to pyterrier", nargs="+", default=[5, 10], required=False
    )
    parser.add_argument(
        "--first-stage-top-k",
        type=int,
        help="top-k documents used for query reformulation",
        default=900,
        required=False,
    )
...
def get_oracle_retrieval_results(topics, oracle_index, overlapping_queries):
    ...
    for _, topic in topics.iterrows():
        r = 0
        for hit in sorted(
            oracle_index[__normalize_queries(overlapping_queries[topic["qid"]])],
    ...
    for wmodel in weighting_models:
        for fb_term in fb_terms:
            for fb_doc in fb_docs:
                ...
                rm3_keyquery_bm25 = (
                    oracle_retrieval_results
                    >> pt.rewrite.RM3(reformulation_index, fb_docs=fb_doc, fb_terms=fb_term)
                    >> pt.BatchRetrieve(index, wmodel=wmodel)
                )

                rm3_keyquery_bm25(topics).to_json(
                    f"{out_dir}/rm3_{wmodel}_{fb_term}_{fb_doc}.jsonl.gz", index=False, lines=True, orient="records"
                )
                ...
                bo1_keyquery_bm25 = (
                    oracle_retrieval_results
                    >> pt.rewrite.Bo1QueryExpansion(reformulation_index, fb_docs=fb_doc, fb_terms=fb_term)
                    >> pt.BatchRetrieve(index, wmodel="BM25")
                )

                bo1_keyquery_bm25(topics).to_json(
                    f"{out_dir}/bo1_{wmodel}_{fb_term}_{fb_doc}.jsonl.gz", index=False, lines=True, orient="records"
                )
                ...
                kl_keyquery_bm25 = (
                    oracle_retrieval_results
                    >> pt.rewrite.KLQueryExpansion(reformulation_index, fb_docs=fb_doc, fb_terms=fb_term)
                    >> pt.BatchRetrieve(index, wmodel="BM25")
                )

                kl_keyquery_bm25(topics).to_json(
                    f"{out_dir}/kl_{wmodel}_{fb_term}_{fb_doc}.jsonl.gz", index=False, lines=True, orient="records"
                )
...
if __name__ == "__main__":
    ...
    bm25_raw = (
        tira.pt.from_submission("ir-benchmarks/tira-ir-starter/BM25 (tira-ir-starter-pyterrier)", args.input_dataset)
