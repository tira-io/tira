...
def setup_logging_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument('-v', '--verbose', default=0, action="count", help='Increases the output verbosity. Default is INFO. Can be repeated for more verbose output.')
...
def setup_download_command(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument('--approach', required=False, default=None, help='Download the outputs of the specified approach. Usage: --approach <task-id>/<user-id>/<approach-name>')
...
def setup_login_command(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument('--print-docker-auth', required=False, default=False, action='store_true', help='Print the docker credentials as TIRA_DOCKER_REGISTRY_USER and TIRA_DOCKER_REGISTRY_TOKEN to stdout.')
...
def main() -> int:
    ...
    if not hasattr(args, 'executable') or args.executable is None:
        ...
        logging.critical(f"Command: {vars(args).get('command', None)}; Executable: {vars(args).get('executable', None)}")
        return -1
